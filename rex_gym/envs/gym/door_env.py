"""This file implements the gym environment of rex open a door task.
...
class RexWalkEnv(rex_gym_env.RexGymEnv):
  ...
  on how far rex goes in opening a door.
  ...
  """
    ...
    def setup_ui(self, base_x, step, period):
        ...
        self.step_period_ui = self._pybullet_client.addUserDebugParameter("step_period", 0., 0.9, period)
    ...
    @staticmethod
    def _evaluate_brakes_stage_coeff(current_t, end_t=0.0, end_value=0.0):
        # ramp function
        p = 0.8
        if end_t <= current_t <= p + end_t:
    ...
    def _IK_signal(self, t, action):
        return self._walk_signal(t)

    def _walk_signal(self, t):
        ...
        if self.goal_reached:
            brakes_coeff = self._evaluate_brakes_stage_coeff(t, self.end_time)
